...
export default function SignupBusinessPage() {
  ...
  const [errors, setErrors] = useState<{ email?: string; password?: string; businessName?: string }>({});

  const validateForm = () => {
    const newErrors: { email?: string; password?: string; businessName?: string } = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { category, email, password, businessName };
    console.log(payload);
  ...
  return (
    <main>
      <div className="relative bg-black backdrop-blur-sm w-full max-w-[650px] rounded-xl p-6 text-white shadow-xl">
        ...
        <h1 className="text-2xl font-bold">Signup Business</h1>
